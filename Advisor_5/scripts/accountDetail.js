...
function downloadFile(location,filename){ 
   // alert("downloadFile: location="+location+", filename="+ filename);   
    //create client directory under Clients folder
    currentDir = root;
    var newDirPath = currentDir.fullPath + "/Clients/" + currentUsername;
  //  alert("newDirPath="+newDirPath);
    currentDir.getDirectory(newDirPath, {create:true, exclusive:false},onGetDirectoryWin, onGetDirectoryFail); 
    
    var url = "http://" + SERVER + "" + location + "/" + filename; 
    var fileTransfer = new FileTransfer();
    var name = filename.substring(filename.lastIndexOf("/") + 1); 
 //   var filePath = root.fullPath + "/Clients/Angela Smith/" + name;    
    var filePath = newDirPath + "/" + name;
  //  alert("dest path" + filePath);
    ...
    function(entry) {
        ...
        var directoryname = currentUsername + DELIMITER + currentInvId;
      //  alert("directoryname="+directoryname);
        sendDirectoryName(directoryname);
        sendFileName(name);
      //  currentDir = root;
